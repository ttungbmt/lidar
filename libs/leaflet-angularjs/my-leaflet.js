...
angular.module('ui-leaflet').factory('leafletMapDefaults', ["$q", "leafletHelpers", function ($q, leafletHelpers) {
  ...
  return {
    ...
    getMapCreationDefaults: function getMapCreationDefaults(scopeId) {
      ...
      if (isDefined(d.contextMenu)) {
        angular.extend(mapDefaults, d.contextMenu);
      }
    ...
    setDefaults: function setDefaults(userDefaults, scopeId) {
      ...
      if (isDefined(userDefaults)) {
        ...
        if (angular.isDefined(L.Map.ContextMenu) && isDefined(userDefaults.contextMenu)) {
          newDefaults.contextMenu = userDefaults.contextMenu;
        }
      }
      var mapId = obtainEffectiveMapId(defaults, scopeId);
...
angular.module('ui-leaflet').service('leafletMarkersHelpers', ["$rootScope", "$timeout", "leafletHelpers", "leafletLogger", "$compile", "leafletGeoJsonHelpers", function ($rootScope, $timeout, leafletHelpers, leafletLogger, $compile, leafletGeoJsonHelpers) {
  ...
  return {
    ...
    listenMarkerEvents: function listenMarkerEvents(marker, markerData, leafletScope, doWatch, map) {
    ...
    addMarkerWatcher: function addMarkerWatcher(marker, name, leafletScope, layers, map, isDeepWatch) {
...
angular.module('ui-leaflet').service('leafletWatchHelpers', function () {
  ...
  var _maybe = function _maybe(scope, watchFunctionName, thingToWatchStr, watchOptions, initCb) {
    ...
    var unWatch = scope[watchFunctionName](thingToWatchStr, function (newValue, oldValue) {
      ...
      if (!watchOptions.doWatch) unWatch();
    }, watchOptions.isDeep);
  ...
  /*
   ...
   to set watching to once and its watch depth.
   */
  var _maybeWatchCollection = function _maybeWatchCollection(scope, thingToWatchStr, watchOptions, initCb) {
...
angular.module('ui-leaflet').directive('decorations', ["leafletLogger", "leafletHelpers", "leafletControlHelpers", function (leafletLogger, leafletHelpers, leafletControlHelpers) {
  var $log = leafletLogger;
  return {
    restrict: "A",
    scope: false,
...
angular.module('ui-leaflet').directive('eventBroadcast', ["leafletLogger", "leafletHelpers", "leafletControlHelpers", function (leafletLogger, leafletHelpers, leafletControlHelpers) {
  var $log = leafletLogger;
  return {
    restrict: "A",
    scope: false,
...
angular.module('ui-leaflet').directive('markers', ["leafletLogger", "$rootScope", "$q", "leafletData", "leafletHelpers", "leafletMapDefaults", "leafletMarkersHelpers", "leafletMarkerEvents", "leafletIterators", "leafletWatchHelpers", "leafletDirectiveControlsHelpers", function (leafletLogger, $rootScope, $q, leafletData, leafletHelpers, leafletMapDefaults, leafletMarkersHelpers, leafletMarkerEvents, leafletIterators, leafletWatchHelpers, leafletDirectiveControlsHelpers) {
  ...
  var _maybeAddMarkerToLayer = function _maybeAddMarkerToLayer(layerName, layers, model, marker, doIndividualWatch, map) {
  ...
  return {
    ...
    link: function link(scope, element, attrs, controller) {
      ...
      mapController.getMap().then(function (map) {
        ...
        // backwards compat
        if (isDefined(attrs.watchMarkers)) watchOptions.doWatch = watchOptions.individual.doWatch = !isDefined(attrs.watchMarkers) || Helpers.isTruthy(attrs.watchMarkers);
...
['markers', 'geojson'].forEach(function (name) {
  angular.module('ui-leaflet').directive(name + 'WatchOptions', ['$log', '$rootScope', '$q', 'leafletData', 'leafletHelpers', function (leafletLogger, $rootScope, $q, leafletData, leafletHelpers) {
      ...
      link: function link(scope, element, attrs, controller) {
        ...
        mapController.getMap().then(function () {
          if (isDefined(scope[name + 'WatchOptions'])) {
            if (isObject(scope[name + 'WatchOptions'])) angular.extend(_watchOptions, scope[name + 'WatchOptions']);else $log.error(errorHeader + '[' + name + 'WatchOptions] is not an object');
...
angular.module('ui-leaflet').factory('leafletContextMenuEvent', ['$rootScope', 'leafletData', 'leafletHelpers', '$timeout', '$log', '$q', '$state', function ($root, leafletData, leafletHelpers, $timeout, $log, $q, $state) {
  var _map,
      self = this,
      measure;
  ...
  self.excute_map = function () {
    leafletData.getMap().then(function (map) {
      _map = map;
      measure = new L.Control.Measure();
      measure.addTo(map);
    });
  };
  ...
  var _measureMap = function _measureMap(index) {
    var item = _map.contextmenu._items[index].el;
    if (L.DomUtil.hasClass(item, 'measure_on')) {
      measure._toggleRuler();
      L.DomUtil.removeClass(item, 'measure_on');
      _map.contextmenu.removeItem(index);
      ...
      L.DomUtil.addClass(_map.contextmenu._items[index].el, 'measure_on');
    }
  };

  var _centerMap = function _centerMap() {
    _map.panTo(_map.contextmenu._showLocation.latlng);
  };

  var _showCoordinates = function _showCoordinates() {
    var location = _map.contextmenu._showLocation.latlng;
  };

  var _zoomRegion = function _zoomRegion($numZoom) {
    angular.isNumber($numZoom) ? _map.setZoom($numZoom) : _map.setZoom(18);
  };

  var _zoomCity = function _zoomCity() {
    _map.setZoom(13);
  };

  var _geolocation = function _geolocation($scope) {
    self.excute_map();
    _map.locate();
    _map.on('locationfound', function (e) {
      _map.fitBounds(e.bounds);

      $scope.layers.overlays.geolocation = {
        name: 'Điểm định vị',
        type: 'group',
        visible: true,
        layerParams: {
      ...
      $scope.markers.geolocation = [];
      $scope.markers.geolocation.push({
        lat: e.latlng.lat,
        lng: e.latlng.lng,
        message: 'Bạn đang ở trong phạm vi <b>' + e.accuracy + 'm</b>',
        icon: {
          type: 'div',
          iconSize: [32, 32],
        ...
        contextmenuItems: [{
          text: 'Thêm điểm',
          iconCls: 'icon-googleplus5'
        }, {
          text: 'Xóa điểm',
          iconCls: 'icon-bin'
        }, '-', {
          text: 'Di chuyển',
          iconCls: 'icon-move-alt1'
        }]
      });
  ...
  var _dragMarker = function _dragMarker($scope) {
    var name_marker = $scope.current_marker.modelName;
    var marker = $scope.markers[name_marker];
    if (!marker.draggable) marker.draggable = true;else {
      console.log('Đang phát triển tắt di chuyển marker');
    }
  };

  var _addMarker = function _addMarker(e) {
    if (_.isUndefined($root.markers.pin)) {
      $root.markers.pin = [];
    }

    $root.markers.pin.push({
      lat: e.latlng.lat,
      lng: e.latlng.lng,
      draggable: true,
      dataForm: {
      ...
      contextmenuItems: [{
        text: 'Chỉnh sửa điểm',
        iconCls: 'icon-pencil6',
        callback: function callback(e) {
          $state.go('home.addDT.csyt');
        }
      }]
    });

    var lastKey = _.size($root.markers.pin);
    $state.go('home.addDT', { stt: lastKey - 1 }, { reload: true });
  };

  return {
    centerMap: _centerMap,
...
//# sourceMappingURL=my-leaflet.js.map
